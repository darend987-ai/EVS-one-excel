...
def unique_sheet_names(base_name: str, idx: int):
    """
    Create two distinct, Excel-safe sheet names for one evidence sample:
      - PL_<hash>_<shortname>
      - SUM_<hash>_<shortname>
    Ensures <=31 chars and uniqueness even if base_name is long.
    """
    safe = "".join(ch if ch not in "\\/?*:[]\"" else "_" for ch in str(base_name)).strip()
    short = safe[:18] if len(safe) > 18 else safe
    h = hashlib.sha1(f"{safe}-{idx}".encode("utf-8")).hexdigest()[:6]
    pl = f"PL_{h}_{short}"[:31]
    sm = f"SUM_{h}_{short}"[:31]
    return pl, sm
...
def run_comparison_from_file(uploaded_file):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        for idx, (_, ev_row) in enumerate(evidence_df.iterrows(), start=1):
            ...
            per_locus_sheet, summary_sheet = unique_sheet_names(ev_name, idx)
